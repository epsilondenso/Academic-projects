...
def nreturns(s: pd.Series) -> pd.Series:
  ...
  return pd.Series(rendimientos[1:], index = s.index[1:])
...
logarítmicos
  ...
  return pd.Series(logret[1:], index = S.index[1:])
...
def minmax(s: pd.Series, feature_range = (0, 1)) -> pd.Series:
  ...
  return pd.Series(scaled.flatten(), index = s.index)
...
tether = pd.read_csv("XRP-2014-2025.csv").iloc[0 ,1:]
xrp = pd.read_csv("Tether-2014-2025.csv").iloc[0 ,1:]
